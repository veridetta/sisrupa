        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title" id="myModalLabel4">Buat Pembayaran Baru</h4>
            ...
            <div class="modal-body">
              <form class="add-new-user pt-0" method="POST" action="{{route('pembayaran-add-admin')}}">
                ...
                <div class="mb-1">
                  <label class="form-label" for="basic-icon-default-id_tagihan">Tagihan</label>
                  <select
                    type="text"
                ...
                <div class="mb-1">
                  <label class="form-label" for="basic-icon-default-id_petugas">Nama Petugas</label>
                  <select
                    type="text"
                    id="basic-icon-default-id_petugas"
                    class="form-control dt-id_petugas"
                    name="id_petugas"
                    placeholder="Agus S"
...
$(function () {
    ...
    if (dt_anggota.length) {
      var dt_ang = dt_anggota.DataTable({
        ...
        columns: [
          ...
          { data: 'keterangan' },
          { data: 'petugas' },
          @if(auth()->user()->role=='admin' || auth()->user()->role=='petugas'){ data: '' }@endif
        ],
        columnDefs: [
          {
            "defaultContent": "-",
  ...
  $('.dt-anggota').on('click', '.a_edit', function () {
        ...
        var url = "//{{request()->getHttpHost()}}/admin/p/pembayaran_data_single/"+dat;
        $.ajax({
          type: "GET",
          url: url,
          success: function(data){
            ...
            $("#basic-icon-default-id_petugas").val(data.data.id_petugas).change();
            $("#basic-icon-default-tanggal").val(data.data.tanggal_pembayaran).change();
