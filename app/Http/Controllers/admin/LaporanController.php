...
class LaporanController extends Controller
{
  ...
  public function bulan(Request $request)
  {
    ...
    return view('layouts/admin/lap_bulanan', ['bulan'=>$nama_bulan,'bulan_a'=>$bulan,'tahun'=>$tahun,'bloks'=>$blok,'val'=>$val,'kars'=>$kar,'pageConfigs' => $pageConfigs, 'breadcrumbs' => $breadcrumbs]);
  }
  
  public function bulan_data(Request $request)
  {
    $user=pembayaran::join('pedagangs','pedagangs.id','=','pembayarans.id_pedagang')->join('jenis','jenis.id','=','pedagangs.jenis')->join('tagihans','tagihans.id','=','pembayarans.id_tagihan')->join('users as p','p.id','=','pedagangs.id_users')->join('users as t','t.id','=','pembayarans.id_petugas')->select('p.name as name','jenis.nama as jenis','pembayarans.tanggal_pembayaran as tanggal','tagihans.nama as tagihan','tagihans.nominal as nominal','pembayarans.keterangan as keterangan','t.name as petugas','pembayarans.id as id','pembayarans.id_pedagang as id_pedagang','pembayarans.id_tagihan as id_tagihan')->whereMonth('pembayarans.tanggal_pembayaran', '=', $request->bulan)
  ...
  public function harian(Request $request)
  {
    ...
    $blok=blok::join('lokasis','lokasis.id','=','bloks.id_lokasi')->orderBy('lokasis.nama')->get();
    return view('layouts/admin/lap_harian', ['tanggal'=>$tanggal,'bloks'=>$blok,'val'=>$val,'kars'=>$kar,'pageConfigs' => $pageConfigs, 'breadcrumbs' => $breadcrumbs]);
  }
  

  public function harian_data(Request $request)
  {
    $user=pembayaran::join('pedagangs','pedagangs.id','=','pembayarans.id_pedagang')->join('jenis','jenis.id','=','pedagangs.jenis')->join('tagihans','tagihans.id','=','pembayarans.id_tagihan')->join('users as p','p.id','=','pedagangs.id_users')->join('users as t','t.id','=','pembayarans.id_petugas')->select('p.name as name','jenis.nama as jenis','pembayarans.tanggal_pembayaran as tanggal','tagihans.nama as tagihan','tagihans.nominal as nominal','pembayarans.keterangan as keterangan','t.name as petugas','pembayarans.id as id','pembayarans.id_pedagang as id_pedagang','pembayarans.id_tagihan as id_tagihan')->where('pembayarans.tanggal_pembayaran', '=', $request->tanggal)->orderBy('pembayarans.tanggal_pembayaran')->get();
    return ['data' => $user];
  }
