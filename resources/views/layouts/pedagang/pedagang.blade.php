        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form class="add-new-user pt-0" method="POST" action="{{route('pedagang-add-pedagang')}}">
                @csrf
...
$(function () {
    ...
    if (dt_anggota.length) {
      var dt_ang = dt_anggota.DataTable({
        ajax: "{{route('pedagang-data-pedagang')}}",
        columns: [
          { data: '' },
          { data: 'name' },
          { data: 'jnama' },
          ...
          { data: 'jk' },
          { data: 'alamat' },
          @if(auth()->user()->role=='pedagang'){ data: '' }@endif
        ],
        columnDefs: [
          {
            "defaultContent": "-",
  ...
  $('.dt-anggota').on('click', '.a_edit', function () {
        ...
        var url = "//{{request()->getHttpHost()}}/pedagang/pedagang_data_single/"+dat;
        $.ajax({
          type: "GET",
          url: url,
