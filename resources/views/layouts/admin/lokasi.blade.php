...
<section id="dashboard-analytics">
  @include('panels.flash')
      <div class="col-lg-12 col-sm-12 col-12">
        ...
        aria-controls="DataTables_Table_0" type="button" data-bs-toggle="modal" data-bs-target="#backdrop"><span>Tambah Lokasi Pasar</span></button>
        @endif
        <div class="card">
          ...
          <div class="card-header bg-primary flex-column align-items-start">
            <div class="row col-12">
                ...
                <div class="col-lg-11 col-10 my-auto">
                    <p class="h4 card-text text-white">Daftar Lokasi Pasar</p>
                </div>
            </div>
          </div>
          <div class="card-body" >
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title" id="myModalLabel4">Tambah Lokasi Pasar</h4>
            ...
            <div class="modal-body">
              <form class="add-new-user pt-0" method="POST" action="{{route('lokasi-add-admin')}}">
                ...
                <div class="mb-1">
                  <label class="form-label" for="basic-icon-default-nama">Nama Pasar</label>
                  <input
                    type="text"
                ...
                <div class="mb-1">
                  <label class="form-label" for="basic-icon-default-lok">Lokasi</label>
                  <textarea
                    type="text"
                    id="basic-icon-default-lok"
                    class="form-control dt-lok"
                    name="lok"
                    placeholder="Cirebon"
                    value="{{old('lok')}}"></textarea>
                  @error('lok')
                      <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                      </span>
...
$(function () {
      ...
      var dt_ang = dt_anggota.DataTable({
        ...
        columnDefs: [
          ...
          }@if(auth()->user()->role=='admin'),
          {
            ...
            orderable: false,
            render: function (data, type, full, meta) {
              return '<div class="text-center"><a class="a_edit btn-sm btn btn-primary" pdf="'+full.id+'">Ubah</a> <a class=" btn-sm a_delete btn btn-primary" pdf="'+full.id+'" href="//{{request()->getHttpHost()}}/admin/m/lokasi_delete/'+full.id+'">Hapus</a></div>';
            }
          }@endif
        ],
  ...
  $('.dt-anggota').on('click', '.a_edit', function () {
        ...
        var url = "//{{request()->getHttpHost()}}/admin/m/lokasi_data_single/"+dat;
        $.ajax({
          type: "GET",
          url: url,
          success: function(data){
            ...
            $("#basic-icon-default-lok").val(data.data.lokasi).change();
